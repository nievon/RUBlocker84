...
- **Запуск и автозапуск через CLI**
  1. Запустите `rucli.exe` (программа автоматически запросит права администратора для изменения настроек сети и файла hosts).
  2. Выберите нужный **preset** для блокировки.
  3. Используйте пункт **3. Install and start service** для установки на автозапуск.
  4. Для временного запуска используйте пункт **4. Start background service (rusvc.exe)** — сервис будет запущен как скрытый фоновый процесс, и его состояние (RUNNING/STOPPED) отображается прямо в меню.
  5. Используйте пункт **5. Kill service process (rusvc.exe)** для принудительной остановки фонового сервиса.
